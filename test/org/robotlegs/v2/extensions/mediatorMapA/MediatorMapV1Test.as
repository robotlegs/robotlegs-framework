...
package org.robotlegs.v2.extensions.mediatorMapA
{
	...
	public class MediatorMapV1Test
	{
		...
		protected var mediatorMapA:MediatorMap;
		...
		public function runBeforeEachTest():void
		{
			...
			mediatorMapA = new MediatorMap(contextView, injector, reflector);

			injector.mapValue(DisplayObjectContainer, contextView);
			injector.mapValue(IInjector, injector);
			injector.mapValue(IEventDispatcher, eventDispatcher);
			injector.mapValue(IMediatorMap, mediatorMapA);
		...
		{
			mediatorMapA.mapView(NonViewComponent, NonViewMediator, null, false, true);
			var nonViewComponent:NonViewComponent = new NonViewComponent();
			var mediator:IMediator = mediatorMapA.createMediator(nonViewComponent);
			...
			Assert.assertTrue('Mediator should have been created for View Component', mediatorMapA.hasMediatorForView(nonViewComponent));
			...
			mediatorMapA.removeMediatorByView(nonViewComponent);
			Assert.assertFalse("Mediator should not exist", mediatorMapA.hasMediator(exactMediator));
			Assert.assertFalse("View Mediator should not exist", mediatorMapA.hasMediatorForView(nonViewComponent));
		}

		[Test]
		public function mediatorIsMappedAndCreatedWithInjectedNonView():void
		{
			mediatorMapA.mapView(NonViewComponent, NonViewMediator, null, false, false);
			var nonViewComponent:NonViewComponent = new NonViewComponent();
			var mediator:IMediator = mediatorMapA.createMediator(nonViewComponent);
			var exactMediator:NonViewMediator = mediator as NonViewMediator;
			Assert.assertNotNull('Mediator should have been created', mediator);
		...
		{
			mediatorMapA.mapView(ViewComponent, ViewMediator);
			mediatorMapA.unmapView(ViewComponent);
			var viewComponent:ViewComponent = new ViewComponent();
			contextView.addChild(viewComponent);
			var hasMediator:Boolean = mediatorMapA.hasMediatorForView(viewComponent);
			var hasMapping:Boolean = mediatorMapA.hasMapping(ViewComponent);
		...
		private function verifyMediatorRemoval(event:Event, data:Object):void
		{
			...
			Assert.assertFalse("Mediator should not exist", mediatorMapA.hasMediator(mediator));
			Assert.assertFalse("View Mediator should not exist", mediatorMapA.hasMediatorForView(viewComponent));
		}
		...
		private function verifyMediatorSurvival(event:Event, data:Object):void
		{
			var viewComponent:ViewComponent = data.view;
			var mediator:IMediator = data.mediator;
			Assert.assertTrue("Mediator should exist", mediatorMapA.hasMediator(mediator));
			Assert.assertTrue("View Mediator should exist", mediatorMapA.hasMediatorForView(viewComponent));
		}
	}
}
